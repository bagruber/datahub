...
Vite 6 -> 8 ist der letzte offene Punkt hier. anagram und learnfarsi laufen bereits auf Vite 8 mit demselben React-19-Stack — das Risiko ist damit belegt niedrig. Zwei Deploy-Workflows (GitHub Pages und Hostinger via FTP); nach dem Update `pnpm run build` **und** `pnpm run build:hostinger` pruefen.
